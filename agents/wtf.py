    ...
    def _update(self, global_step, start_time, next_obs, next_done):
        ...
        for epoch in range(self.config.update_epochs):
            ...
            for start in range(0, self.batch_size, self.minibatch_size):
                ...
                # Policy loss
                coeff = max(self.config.coeff_min, self.config.coeff_max - (self.decay * global_step
                                                                            / self.config.total_timesteps))
                lower_bound = coeff * ratio + (1 - coeff) * (1 - self.config.clip_coef)
                upper_bound = coeff * ratio + (1 - coeff) * (1 + self.config.clip_coef)
